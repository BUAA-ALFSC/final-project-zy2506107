...
def _trim_preface_of_body(sample: str) -> str:
    ...
    for lineno, line in enumerate(lines):
        if line.strip():
            first_code_lineno = lineno
            break
    if first_code_lineno is not None and lines[first_code_lineno].startswith('def '):
        lines = lines[first_code_lineno + 1:]
    cleaned = []
    for line in lines:
        stripped = line.strip()
        if not stripped:
            if cleaned:
                cleaned.append(line)
            continue
        if line.startswith(('def ', 'class ', 'import ', 'from ', 'if __name__')):
            break
    ...
    if non_comment_indents:
        min_indent = min(indent for indent in non_comment_indents if indent > 0)
        normalised = []
        for line in cleaned:
            if not line.strip():
                normalised.append(line)
            elif line.lstrip().startswith('#') and len(line) - len(line.lstrip(' ')) < min_indent:
    ...
    if 'return' not in body:
        return '    return -np.abs(bins - item)\n'
    return body + '\n'


def _append_qwen_sample(raw: str, trimmed: str) -> None:
    log_path = os.environ.get('QWEN_SAMPLE_LOG')
    if not log_path:
        return
    record = {'time': time.time(), 'raw': raw, 'trimmed': trimmed}
...
class LocalLLM(sampler.LLM):
            ...
            try:
                ...
                if self._trim:
                    ...
                    for sample in all_samples:
                        ...
                        _append_qwen_sample(sample, trimmed)
                        trimmed_samples.append(trimmed)
                    all_samples = trimmed_samples

                return all_samples
            except Exception as exc:
                print(f'LocalLLM request failed: {type(exc).__name__}: {exc}')
        return ['    return -np.abs(bins - item)\n' for _ in range(self._samples_per_prompt)]
    ...
    def _do_request(self, content: str) -> str:
        ...
        response = requests.post(self._url, data=json.dumps(data), headers=headers, timeout=900)
